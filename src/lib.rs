                    ...
                    match arg.ty.as_ref() {
                                        ...
                                        GenericArgument::Type(ty) => {
                                            match ty {
                                                Type::Reference(ty) => {
                                                    ...
                                                    if mutable {
                                                        query.push(
                                                            parse_quote!(::amethyst::prelude::TryWrite<#elem>),
                                                        );
                                                    } else {
                                                        query.push(
                                                            parse_quote!(::amethyst::prelude::TryRead<#elem>),
                                                        );
                                                    }
                                                }
                                                _ => {
                        ...
                        Type::Path(ty_path)
                            if path_match(ty_path, &["Query"])
                                || path_match(ty_path, &["amethyst", "ecs", "Query"])
                                || path_match(ty_path, &["amethyst", "ecs", "query", "Query"])
                                || path_match(ty_path, &["amethyst", "prelude", "Query"])
                                || path_match(ty_path, &["amethyst", "prelude", "query", "Query"]) =>
                        {
                        ...
                        Type::Reference(ty)
                            if is_type(&ty.elem, &["CommandBuffer"])
                                || is_type(&ty.elem, &["amethyst", "prelude", "CommandBuffer"])
                                || is_type(&ty.elem, &["amethyst", "prelude", "systems", "CommandBuffer"])
                                || is_type(&ty.elem, &["amethyst", "ecs", "systems", "CommandBuffer"])
                                || is_type(&ty.elem, &["amethyst", "ecs", "systems", "CommandBuffer"]) =>
                        {
                        ...
                        Type::Reference(ty)
                            if is_type(&ty.elem, &["SubWorld"])
                                || is_type(&ty.elem, &["amethyst", "prelude", "SubWorld"])
                                || is_type(&ty.elem, &["amethyst", "prelude", "world", "SubWorld"])
                                || is_type(&ty.elem, &["amethyst", "ecs", "SubWorld"])
                                || is_type(&ty.elem, &["amethyst", "ecs", "world", "SubWorld"]) =>
                        {
                            if ty.mutability.is_some() {
                        ...
                        Type::Reference(ty)
                            if is_type(&ty.elem, &["Entity"])
                                || is_type(&ty.elem, &["amethyst", "prelude", "Entity"])
                                || is_type(&ty.elem, &["amethyst", "prelude", "world", "Entity"])
                                || is_type(&ty.elem, &["amethyst", "ecs", "Entity"])
                                || is_type(&ty.elem, &["amethyst", "ecs", "world", "Entity"]) =>
                        {
                            parameters.push(Parameter::Component(query.len()));
                            query.push(parse_quote!(::amethyst::prelude::Entity));
                        }
                        Type::Reference(ty)
                            if is_type(&ty.elem, &["Query"])
                                || is_type(&ty.elem, &["amethyst", "prelude", "Query"])
                                || is_type(&ty.elem, &["amethyst", "prelude", "query", "Query"])
                                || is_type(&ty.elem, &["amethyst", "ecs", "Query"])
                                || is_type(&ty.elem, &["amethyst", "ecs", "query", "Query"]) =>
                        {
                        ...
                        Type::Reference(ty) => {
                            ...
                            match attribute {
                                ...
                                None => {
                                    ...
                                    let elem = &ty.elem;
                                    if mutable {
                                        query.push(parse_quote!(::amethyst::prelude::Write<#elem>));
                                    } else {
                                        query.push(parse_quote!(::amethyst::prelude::Read<#elem>));
                                    }
                                }
                            }
                        }
...
impl Config {
    ...
    fn generate(&mut self) -> Result<proc_macro2::TokenStream, Error> {
        ...
        let builder = quote! {
            use ::amethyst::prelude::IntoQuery;
            #generic_parameter_names
            ::amethyst::prelude::SystemBuilder::new(format!("{}{}", #system_name, generic_names))
        ...
        let result = quote! {
            #visibility fn #constructor_name<#generic_params>(#(#fn_params),*) -> impl ::amethyst::prelude::Runnable
            #where_clause
            {
                #builder
            }
